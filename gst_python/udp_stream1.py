...
class VideoPlayer:
	def __init__(self, vidSocket):
		...
		self.queue.set_property('min-threshold-time', 100000)

		self.tsparse = Gst.ElementFactory.make('tsparse', None)			# parse mpeg-2 stream into packets of set size
		self.pipeline.add(self.tsparse)

		self.demux = Gst.ElementFactory.make('tsdemux', None)			# remove audio from mpeg-2 stream
		self.demux.connect("pad-added", self.demux_added)
		self.pipeline.add(self.demux)

		#self.queue = Gst.ElementFactory.make('queue', 'queue')			# add to queue
		#self.pipeline.add(self.queue)
