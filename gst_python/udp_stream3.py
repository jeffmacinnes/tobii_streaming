...
class VideoPlayer:
	def __init__(self, vidSocket):
		...
		self.vidSink = Gst.ElementFactory.make('autovideosink', None)
		self.pipeline.add(self.vidSink)

		# link all pipeline elements that can be linked without a connection
		self.udpsrc.link_filtered(self.buffer, Gst.caps_from_string("application/x-rtp, payload=96"))
		self.buffer.link(self.depay)
		self.depay.link(self.avdec_h264)
		self.avdec_h264.link(self.vidSink)
